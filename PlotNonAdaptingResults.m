function [] = PlotNonAdaptingResults(res,fig_num)
...
for finalIndexer = 1:(numSSCycles)
    ...
    desPhiThisLoop = res.phiDRR.Data(useThisRange); % radians
    ...
    myPhiD   = desPhiThisLoop(gcThisLoop==1); % radians
    myGRFDir = atan2d(myGRFz,myGRFx);

    GRFDir = atan2d(GRFzThisLoop,GRFxThisLoop);
    timePercentage = ((myTime-myTime(1))/(myTime(end)-myTime(1)))*100;
    ...
    desPhiAccumulator{accumulationCounter} = myPhiD; % radians
...
for l = 1:smallestLength % for every time step
    ...
    for k = 1:length(KgAccumulator) % for every stance phase
        ...
        phiDPerTimeStepVector(k) = desPhiAccumulator{k}(l); % radians
    end
    ...
    phiDAvePerTimestep(l) = mean(phiDPerTimeStepVector); % radians
end
...
%% Plotter
% Plot the average GRF as a thick line. Surround the average
...
hold on
plot(x, GRFDirAvePerTimestep,'b--','LineWidth',3,'DisplayName','$\phi$ NonAdapting')
fill([x, flip(x)], [(GRFDirAvePerTimestep+GRFDirStdPerTimestep), flip((GRFDirAvePerTimestep-GRFDirStdPerTimestep))], 'b-','FaceAlpha',0.2,'LineWidth',1,'HandleVisibility','off')
...
res.RMSDphi = rmsd;
fprintf('RMSD NonAdapting was %0.4f \n',rmsd)
